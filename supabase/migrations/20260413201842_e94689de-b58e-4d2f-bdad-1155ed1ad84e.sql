...
CREATE FUNCTION public.get_guest_booking_by_token(_token text)
RETURNS TABLE(
  id uuid,
  pickup_address text,
...
BEGIN
  ...
  SELECT 
    ...
    c.status::text,
    c.guest_name,
    COALESCE(di.devis_amount, c.final_payment_amount, c.guest_estimated_price) as guest_estimated_price,
    md.d_name,
    md.d_company,
    md.d_phone,
    md.d_avatar,
