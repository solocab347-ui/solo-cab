...
BEGIN
  SELECT json_build_object(
    ...
    'total_fees_solocab', COALESCE((
      ...
      SELECT SUM(COALESCE(sal.fee_amount, 0))
      FROM solo_admin_ledger sal
      WHERE sal.created_at BETWEEN v_start AND v_end
    ), 0) + COALESCE((
      SELECT SUM(COALESCE(dfl.amount_cents, 0))::numeric / 100
      FROM driver_fees_ledger dfl
      WHERE dfl.created_at BETWEEN v_start AND v_end
    ), 0),
    'total_pending_drivers', COALESCE((
      SELECT SUM(COALESCE(dbp.net_amount, 0))
