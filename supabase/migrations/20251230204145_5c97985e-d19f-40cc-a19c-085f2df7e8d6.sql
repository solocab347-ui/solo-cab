-- Supprimer et recréer la fonction pour inclure tous les gestionnaires visibles
DROP FUNCTION IF EXISTS public.get_visible_fleet_managers();

CREATE FUNCTION public.get_visible_fleet_managers()
 RETURNS TABLE(id uuid, company_name text, contact_name text, contact_email text, logo_url text, description text, driver_profile_description text, default_partnership_commission numeric, address text, total_drivers integer, total_clients integer, services_offered text[], partnership_terms text, show_contact_name boolean, show_address boolean, show_phone boolean, show_email boolean, contact_phone text)
 LANGUAGE sql
