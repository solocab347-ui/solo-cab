...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'courses_no_payment', (
      ...
      WHERE c.status = 'completed'
        ...
        AND NOT EXISTS (
          ...
          WHERE p.course_id = c.id AND p.status IN ('succeeded', 'captured')
        )
    ),
    'courses_period', (
    ...
    'spontaneous_period', (
      SELECT count(*) FROM public.payments p
      WHERE p.status IN ('succeeded', 'captured') AND p.payment_type = 'spontaneous'
        AND COALESCE(p.captured_at, p.created_at) BETWEEN v_period_start AND v_period_end
    ),
    'drivers_active_period', (
      SELECT count(DISTINCT p.driver_id) FROM public.payments p
      WHERE p.status IN ('succeeded', 'captured')
        AND COALESCE(p.captured_at, p.created_at) BETWEEN v_period_start AND v_period_end
    ),
