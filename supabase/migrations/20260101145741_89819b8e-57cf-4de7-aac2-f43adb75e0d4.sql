-- Mettre à jour la vue drivers_available_for_sharing pour inclure show_rating_partners
DROP VIEW IF EXISTS drivers_available_for_sharing;

CREATE VIEW drivers_available_for_sharing AS
SELECT 
  d.id,
  d.user_id,
  d.sharing_number,
...
-- Supprimer d'abord la fonction existante pour pouvoir changer sa signature
DROP FUNCTION IF EXISTS public.search_available_partners(uuid,text,text,numeric);

-- Recréer la fonction avec le nouveau champ show_rating_partners
CREATE FUNCTION public.search_available_partners(
  _driver_id uuid,
