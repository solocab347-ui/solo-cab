...
DROP FUNCTION IF EXISTS calculate_fleet_course_price(UUID, NUMERIC, NUMERIC, BOOLEAN, TIMESTAMP, TEXT, TEXT);
DROP FUNCTION IF EXISTS calculate_fleet_course_price(UUID, NUMERIC, NUMERIC, BOOLEAN, TIMESTAMPTZ, TEXT, TEXT);
DROP FUNCTION IF EXISTS calculate_fleet_course_price(UUID, NUMERIC, NUMERIC, BOOLEAN, TEXT, TEXT);

-- Recréer une seule version propre avec TIMESTAMPTZ
...
BEGIN
  ...
  IF v_fm IS NULL THEN
    RAISE EXCEPTION 'Fleet manager not found: %', p_fleet_manager_id;
  END IF;
  ...
  IF p_pickup_city IS NOT NULL AND p_destination_city IS NOT NULL 
    ...
    WHERE fleet_manager_id = p_fleet_manager_id 
      AND is_active = true 
      AND LOWER(city_name) = LOWER(TRIM(p_pickup_city))
