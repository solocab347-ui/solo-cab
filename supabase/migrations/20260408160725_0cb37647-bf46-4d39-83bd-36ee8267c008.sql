...
-- Fix: calculer stripe_fee quand payment_method est stripe/card même sans stripe_payment_intent_id
...
BEGIN
  ...
  -- ═══════════════════════════════════════════════════════
  -- STRIPE FEE LOGIC - FIX: detect card payments by method
  -- ═══════════════════════════════════════════════════════
  ...
  ELSIF v_payment_method IN ('stripe', 'card', 'carte', 'cb') OR NEW.stripe_payment_intent_id IS NOT NULL THEN
    -- Card payment: estimate Stripe fees (1.5% + 0.25€)
  ...
  IF NEW.net_to_driver IS NOT NULL AND NEW.net_to_driver > 0 THEN
    v_net := NEW.net_to_driver;
  END IF;
...
-- Recalculate existing records by re-triggering
UPDATE payments SET updated_at = now() WHERE status IN ('succeeded', 'captured') AND course_id IS NOT NULL AND driver_id IS NOT NULL;
