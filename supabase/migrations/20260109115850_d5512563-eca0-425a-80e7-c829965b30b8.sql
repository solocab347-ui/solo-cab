-- 1. Amélioration du trigger create_expense_report_for_employee_payment
-- Couvre maintenant les cas paid_on_spot et employee_paid_spot
...
BEGIN
  ...
  -- IMPROVED: Check multiple conditions for employee-paid scenarios
  ...
  -- 2. NEW: When actual_payment_method indicates employee paid
  IF v_company_course.actual_payment_method IN ('employee_paid_spot', 'employee_personal', 'paid_on_spot') THEN
    v_should_create_expense := true;
  END IF;
  
  -- 3. NEW: When client confirmed paid_on_spot
  IF v_company_course.client_confirmed_payment_method = 'paid_on_spot' THEN
    v_should_create_expense := true;
  END IF;
...
-- 2. Créer une fonction pour créer une note de frais manuellement (pour les confirmations employé)
CREATE OR REPLACE FUNCTION create_expense_report_for_course(
  p_course_id UUID,
  p_employee_id UUID,
  p_amount NUMERIC
)
...
DECLARE
  ...
  v_expense_id UUID;
BEGIN
  -- Vérifier que la note de frais n'existe pas déjà
  IF EXISTS (SELECT 1 FROM public.expense_reports WHERE course_id = p_course_id) THEN
    -- Retourner l'ID existant
    SELECT id INTO v_expense_id FROM public.expense_reports WHERE course_id = p_course_id;
    RETURN v_expense_id;
  END IF;
  ...
  SELECT * INTO v_employee FROM public.company_employees WHERE id = p_employee_id;
  IF NOT FOUND THEN
  ...
  INSERT INTO public.expense_reports (
    company_id, employee_id, course_id,
    amount, payment_method, description, status, submitted_at
  ) VALUES (
    v_employee.company_id, p_employee_id, p_course_id,
    p_amount,
    COALESCE(v_course.payment_method_used, 'card'),
    'Course VTC du ' || to_char(v_course.scheduled_date, 'DD/MM/YYYY'),
    'pending', now()
  )
  RETURNING id INTO v_expense_id;
  
  -- Mettre à jour les stats employé
  UPDATE public.company_employees 
  SET current_month_spent = COALESCE(current_month_spent, 0) + p_amount,
      monthly_courses_count = COALESCE(monthly_courses_count, 0) + 1
