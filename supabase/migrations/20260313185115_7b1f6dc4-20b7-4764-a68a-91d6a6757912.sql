-- 1) DATA HYGIENE: remove invalid/duplicate daily entries that can inflate stats

-- Remove invalid external rows (external entries must always have a platform_id)
DELETE FROM public.driver_daily_entries
WHERE COALESCE(is_solocab, false) = false
  AND platform_id IS NULL;

-- Deduplicate SoloCab rows: keep latest per (driver_id, entry_date)
WITH ranked_solocab AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY driver_id, entry_date
      ORDER BY updated_at DESC NULLS LAST, created_at DESC NULLS LAST, id DESC
    ) AS rn
  FROM public.driver_daily_entries
  WHERE COALESCE(is_solocab, false) = true
    AND platform_id IS NULL
)
DELETE FROM public.driver_daily_entries d
USING ranked_solocab r
WHERE d.id = r.id
  AND r.rn > 1;

-- Deduplicate external rows: keep latest per (driver_id, entry_date, platform_id)
WITH ranked_external AS (
  SELECT
    id,
    ROW_NUMBER() OVER (
      PARTITION BY driver_id, entry_date, platform_id
      ORDER BY updated_at DESC NULLS LAST, created_at DESC NULLS LAST, id DESC
    ) AS rn
  FROM public.driver_daily_entries
  WHERE COALESCE(is_solocab, false) = false
...
DELETE FROM public.driver_daily_entries d
USING ranked_external r
WHERE d.id = r.id
  AND r.rn > 1;

-- Enforce consistency for future inserts/updates
ALTER TABLE public.driver_daily_entries
  DROP CONSTRAINT IF EXISTS driver_daily_entries_platform_consistency_chk;

ALTER TABLE public.driver_daily_entries
  ADD CONSTRAINT driver_daily_entries_platform_consistency_chk
  CHECK (
    (COALESCE(is_solocab, false) = true AND platform_id IS NULL)
    OR
    (COALESCE(is_solocab, false) = false AND platform_id IS NOT NULL)
  );

-- Enforce uniqueness to prevent future duplicates
CREATE UNIQUE INDEX IF NOT EXISTS idx_driver_daily_entries_unique_solocab_day
  ON public.driver_daily_entries (driver_id, entry_date)
  WHERE COALESCE(is_solocab, false) = true;

CREATE UNIQUE INDEX IF NOT EXISTS idx_driver_daily_entries_unique_external_platform_day
  ON public.driver_daily_entries (driver_id, entry_date, platform_id)
  WHERE COALESCE(is_solocab, false) = false
    AND platform_id IS NOT NULL;


-- 2) HARDEN DRIVER DASHBOARD AGGREGATION (exclude tests/simulations/ghost completed rows)
...
BEGIN
  WITH direct_completed AS (
    SELECT
      c.id AS course_id,
      (timezone('Europe/Paris', COALESCE(c.scheduled_date, c.payment_confirmed_at, c.updated_at, c.created_at)))::date AS activity_day,
    ...
    LEFT JOIN LATERAL (
      SELECT d.amount, d.quote_number
      FROM public.devis d
    ...
    WHERE (c.driver_id = p_driver_id OR p_driver_id = ANY(COALESCE(c.driver_ids, '{}'::uuid[])))
      ...
      AND COALESCE(c.course_number, '') !~* '^(TEST|SIM|DEMO|DEV)[-_]'
      AND COALESCE(dq.quote_number, '') !~* '^(DEV-TEST|TEST|SIM|DEMO)[-_]'
      AND COALESCE(c.final_payment_amount, c.guest_estimated_price, dq.amount, 0) > 0
      AND NOT EXISTS (
        SELECT 1
        FROM public.shared_courses scx
        WHERE scx.course_id = c.id
          AND scx.status = 'completed'
          AND scx.receiver_driver_id IS DISTINCT FROM p_driver_id
      )
  ),
  shared_completed AS (
    SELECT
      sc.course_id,
      (timezone('Europe/Paris', COALESCE(sc.completed_at, c.scheduled_date, c.payment_confirmed_at, c.updated_at, c.created_at, sc.created_at)))::date AS activity_day,
    ...
    WHERE sc.receiver_driver_id = p_driver_id
      ...
      AND COALESCE(c.course_number, '') !~* '^(TEST|SIM|DEMO|DEV)[-_]'
      AND COALESCE(dq.quote_number, '') !~* '^(DEV-TEST|TEST|SIM|DEMO)[-_]'
      AND COALESCE(sc.earnings_for_receiver, c.final_payment_amount, c.guest_estimated_price, dq.amount, sc.course_amount, 0) > 0
  ),
  ...
  client_scope AS (
    SELECT (timezone('Europe/Paris', created_at))::date AS created_day
    FROM public.clients
  ...
  external_dedup AS (
    SELECT
      ...
      ROW_NUMBER() OVER (
        PARTITION BY e.entry_date, e.platform_id
...
CREATE OR REPLACE FUNCTION public.get_driver_solocab_day_stats(p_driver_id uuid, p_day date)
RETURNS TABLE(
  revenue numeric,
...
AS $function$
  ...
  day_platform AS (
    SELECT
      COALESCE(SUM(effective_revenue), 0)::numeric AS revenue,
      COUNT(*)::int AS courses_count,
      (COALESCE(SUM(duration_minutes), 0) / 60.0)::numeric AS hours_worked,
      COALESCE(SUM(distance_km), 0)::numeric AS km_driven
    FROM platform_completed_scope
    WHERE activity_day = p_day
  ),
  day_clients AS (
    SELECT COUNT(*)::int AS new_clients_count
    FROM public.clients cl
    WHERE (cl.driver_id = p_driver_id OR p_driver_id = ANY(COALESCE(cl.driver_ids, '{}'::uuid[])))
      AND (timezone('Europe/Paris', cl.created_at))::date = p_day
  )
  SELECT
    dp.revenue,
    dp.courses_count,
    dc.new_clients_count,
    dp.hours_worked,
    dp.km_driven
  FROM day_platform dp
  CROSS JOIN day_clients dc;
$function$;