-- Harmonize driver dashboard stats and daily SoloCab sync source
-- 1) Fix aggregation consistency (direct vs shared), exclude test/simulation artifacts
-- 2) Add per-day RPC used by frontend auto-sync to avoid timezone and field mismatch bugs
...
BEGIN
  WITH direct_completed AS (
    ...
    WHERE (c.driver_id = p_driver_id OR p_driver_id = ANY(COALESCE(c.driver_ids, '{}'::uuid[])))
      ...
      AND COALESCE(c.course_number, '') NOT ILIKE 'TEST-%'
      AND COALESCE(dq.quote_number, '') NOT ILIKE 'DEV-TEST-%'
      -- If course was delegated and completed by another receiver driver, exclude from direct stats
      AND NOT EXISTS (
        SELECT 1
