...
DECLARE
  ...
  v_favorite_radius_m integer := 5000; -- 5km guaranteed radius for favorites
...
BEGIN
  ...
  ranked_drivers AS (
    ...
    WHERE 
      -- Include if within normal search radius
      ST_DistanceSphere(
        ST_MakePoint(cd.candidate_longitude, cd.candidate_latitude),
        ST_MakePoint(p_longitude, p_latitude)
      ) <= v_requested_radius_m
      -- OR include if favorite within 5km (even if outside normal radius)
      OR (
        cd.is_favorite
