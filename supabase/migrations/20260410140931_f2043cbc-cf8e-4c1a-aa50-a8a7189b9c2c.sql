...
BEGIN
  ...
  WITH candidate_drivers AS (
    SELECT
      ...
      CASE
        WHEN v_mode = 'immediate' THEN d.current_latitude::double precision
        ELSE d.home_latitude::double precision
      END AS candidate_latitude,
      CASE
        WHEN v_mode = 'immediate' THEN d.current_longitude::double precision
        ELSE d.home_longitude::double precision
      END AS candidate_longitude,
      CASE
        WHEN v_mode = 'immediate' THEN true
        ELSE false
