DROP FUNCTION IF EXISTS public.get_guest_booking_by_token(text);
...
RETURNS TABLE(
  ...
  status text,
  guest_name text,
  guest_estimated_price double precision,
  driver_name text,
  driver_company text,
  driver_phone text,
  driver_avatar text,
  ...
  devis_amount double precision,
  quote_number text,
  final_payment_amount double precision,
  ...
  facture_id uuid,
  facture_number text,
  facture_amount double precision,
...
BEGIN
  ...
  facture_info AS (
    SELECT
      ...
      f.payment_status as f_payment_status,
      f.course_id
    FROM factures f
  ...
  SELECT 
    ...
    c.status::text,
    c.guest_name,
    COALESCE(di.devis_amount, c.final_payment_amount, c.guest_estimated_price),
    md.d_name,
    md.d_company,
    md.d_phone,
    md.d_avatar,
