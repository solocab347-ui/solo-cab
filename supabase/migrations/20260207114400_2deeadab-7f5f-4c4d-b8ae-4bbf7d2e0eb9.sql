-- Ajouter la colonne trial_started_at à la table drivers
ALTER TABLE public.drivers 
ADD COLUMN IF NOT EXISTS trial_started_at timestamp with time zone;

-- Copier les données de trial_activated_at vers trial_started_at si elle existe
UPDATE public.drivers 
SET trial_started_at = trial_activated_at 
WHERE trial_activated_at IS NOT NULL AND trial_started_at IS NULL;

-- Recréer la fonction RPC pour corriger l'erreur
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(d.wants_tpe_affiliate, false),
    d.tpe_received_at,
    COALESCE(d.trial_started_at, d.trial_activated_at),
    COALESCE(d.trial_ready_to_start, false),
    COALESCE(d.objectives_completed, false),
    COALESCE(d.onboarding_objectives_completed, false),
    d.onboarding_step,
...
-- Corriger la politique RLS pour security_audit_logs
