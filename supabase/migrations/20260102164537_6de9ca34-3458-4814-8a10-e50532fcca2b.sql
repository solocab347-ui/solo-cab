-- Recréer la fonction accept_devis_safely avec une signature claire
DROP FUNCTION IF EXISTS public.accept_devis_safely(uuid, uuid);

CREATE OR REPLACE FUNCTION public.accept_devis_safely(
  _devis_id uuid,
...
BEGIN
  ...
  v_is_driver_created := (SELECT created_by_user_id FROM courses WHERE id = v_course_id) = v_devis.driver_user_id;
  ...
  ELSE
    ...
    VALUES (
      ...
      'Le client a accepté le devis ' || COALESCE(v_devis.quote_number, 'N/A') || '. Vous devez maintenant accepter la course.',
      'devis_accepted',
      '/driver-dashboard?tab=courses'
    );
