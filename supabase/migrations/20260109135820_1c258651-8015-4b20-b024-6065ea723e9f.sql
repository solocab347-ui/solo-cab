...
BEGIN
  ...
  IF v_city_pricing_found THEN
    ...
    -- Calculate TVA
    IF COALESCE(v_city_pricing.tva_included, false) THEN
      v_tva := v_subtotal - (v_subtotal / (1 + COALESCE(v_city_pricing.tva_rate, 10) / 100));
    ELSE
      v_tva := v_subtotal * (COALESCE(v_city_pricing.tva_rate, 10) / 100);
    END IF;
  ELSE
    ...
    IF COALESCE(v_fm.tva_included, false) THEN
      v_tva := v_subtotal - (v_subtotal / (1 + COALESCE(v_tva_rate, 10) / 100));
    ELSE
      v_tva := v_subtotal * (COALESCE(v_tva_rate, 10) / 100);
    END IF;
  END IF;
  ...
  RETURN QUERY SELECT 
    ...
    v_subtotal,
    v_tva,
    CASE WHEN COALESCE(v_fm.tva_included, false) OR v_city_pricing_found AND COALESCE(v_city_pricing.tva_included, false)
         THEN v_subtotal 
         ELSE v_subtotal + v_tva 
    END,
    v_evening,
    v_weekend,
    v_pricing_source;
