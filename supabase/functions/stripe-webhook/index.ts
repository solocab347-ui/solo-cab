  ...
  try {
          ...
          try {
            ...
            if (driverData?.user_id) {
              await supabaseClient.from("notifications").insert({
                ...
                message: `Votre paiement de 29,99€ n'a pas pu être effectué (tentative ${attemptCount}). Mettez à jour votre carte pour éviter la suspension.`,
                priority: "high",
                action_url: "/driver-dashboard?tab=subscription",
              });
    ...
    if (event.type === "checkout.session.completed") {
      ...
      if (metadata.type === "shared_course_payment" && metadata.shared_course_id) {
        ...
        // New commission model: fixed 15%(<30€) / 20%(≥30€) + 0.20€ SoloCab fee
        const commissionAmount = parseFloat(metadata.commission_amount || "0");
        const solocabFee = parseFloat(metadata.solocab_fee || "0.20");
