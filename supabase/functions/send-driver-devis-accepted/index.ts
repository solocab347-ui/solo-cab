...
serve(async (req) => {
  ...
  try {
    ...
    const { driver_id, client_name, devis_amount, course_date } = await req.json();

    if (!driver_id || !client_name || !devis_amount) {
      return new Response(
    ...
    console.log(`📧 Envoi email devis accepté à ${driverEmail}`);
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
            ...
            <div style="background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px;">
              ...
              <p>Excellente nouvelle ! <strong>${client_name}</strong> a accepté votre devis.</p>
              
              <div style="background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981;">
                <p><strong>💰 Montant :</strong> ${devis_amount}€</p>
                ${course_date ? `<p><strong>📅 Date :</strong> ${course_date}</p>` : ''}
              </div>
              
              <p>🎯 <strong>Prochaines étapes :</strong></p>
              <ul>
                <li>La course est maintenant confirmée</li>
                <li>Vous pouvez communiquer avec le client via la messagerie</li>
                <li>N'oubliez pas de terminer la course après réalisation pour générer la facture</li>
              </ul>
    ...
    if (!emailResult.success) {
      ...
      await sendAdminAlert(resend, {
        emailType: "driver_devis_accepted",
        recipient: driverEmail,
        error: emailResult.error || "Erreur inconnue",
        context: `Driver ID: ${driver_id}, Client: ${client_name}, Amount: ${devis_amount}€`
      });
    } else {
