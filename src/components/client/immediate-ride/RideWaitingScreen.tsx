...
export function RideWaitingScreen({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 3D Driver Carousel */}
      {(status === 'searching' || status === 'extended_searching' || status === 'relaunching' || status === 'transition' || acceptedDriverId) && (
        <div className="space-y-2">
          <h4 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide text-center">
            Chauffeurs contactés ({effectiveDriverCount})
          </h4>
          <DriverCarousel3D
            drivers={contactedDrivers.map(d => ({
              driver_id: d.driver_id,
              driver_name: d.driver_name,
              photo_url: d.photo_url,
              status: d.status,
              estimated_price: contactedDriversData?.find(cd => cd.driver_id === d.driver_id)?.estimated_price,
            }))}
            acceptedDriverId={acceptedDriverId}
            isSearching={status === 'searching' || status === 'extended_searching' || status === 'relaunching' || status === 'transition'}
          />
        </div>
      )}
