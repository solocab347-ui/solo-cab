...
interface CompanyEmployeeFacturesProps {
  employeeId: string;
  companyName: string;
}

export const CompanyEmployeeFactures = ({ employeeId, companyName }: CompanyEmployeeFacturesProps) => {
  ...
  const fetchFactures = async () => {
    try {
      // Récupérer uniquement les factures liées aux courses de cet employé
      const { data: employeeCourses, error: coursesError } = await supabase
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Factures liées à vos courses personnelles
        </p>
      </div>
