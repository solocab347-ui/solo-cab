...
export const DriverFleetPartnerships = ({ driverId }: DriverFleetPartnershipsProps) => {
  ...
  const submitProposal = async () => {
    ...
    try {
      ...
      // Vérifier si un partenariat existe déjà (tous statuts)
      ...
      if (existingPartnership) {
        // Si le partenariat est actif ou en attente, bloquer
        if (existingPartnership.status === "pending" || existingPartnership.status === "accepted") {
          toast.error("Un partenariat existe déjà avec ce gestionnaire");
          setSubmitting(false);
          return;
        }

        // Sinon, réactiver le partenariat existant (rejected/terminated)
  ...
  const handleRejectPartnership = async (reason: string, shouldBlock: boolean) => {
    ...
    try {
      ...
      // Block if requested
      if (shouldBlock) {
        await blockEntity(rejectingPartnership.fleet_manager_id, reason);
        toast.success("Partenariat refusé et gestionnaire bloqué");
      } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <PartnershipRejectDialog
        ...
        partnerName={rejectingPartnership?.fleet_manager?.company_name || 'Gestionnaire de flotte'}
        partnerType="company"
