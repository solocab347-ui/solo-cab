    ...
    <div className="w-full max-w-7xl mx-auto px-2 sm:px-4 lg:px-8 py-4 sm:py-6 space-y-6 sm:space-y-8">
      ...
      <div className="animate-fade-in">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4 lg:gap-6">
          ...
          {/* Encaissement spontané - Premium */}
          <Card
            className={`relative overflow-hidden p-4 sm:p-6 lg:p-8 bg-gradient-to-br from-card/80 via-card/60 to-card/80 backdrop-blur-xl transition-all border ${isFree ? 'border-amber-500/30' : 'border-border/50'} shadow-lg group touch-manipulation ${
              !isFree && driverProfile?.driver?.stripe_connect_charges_enabled
                ? 'hover:scale-[1.02] cursor-pointer active:scale-[0.98]'
                : isFree ? 'hover:scale-[1.02] cursor-pointer active:scale-[0.98]' : 'opacity-60 cursor-not-allowed'
            }`}
            {...(isFree
              ? getTapProps<HTMLDivElement>(() => onTabChange("subscription"))
            ...
            <div className="relative z-10 flex flex-col items-center text-center space-y-2 sm:space-y-4">
              <div className={`w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 rounded-xl sm:rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform ${
                !isFree && driverProfile?.driver?.stripe_connect_charges_enabled
                  ...
                  : isFree ? 'bg-gradient-to-br from-green-500 to-emerald-600' : 'bg-muted'
              }`}>
                {isFree ? (
                  <Lock className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-white" />
                ) : (
